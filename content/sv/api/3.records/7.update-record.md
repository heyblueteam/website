...
För att uppdatera en posts kärnegenskaper, använd mutation `editTodo`:
...
|-------|------|-------------|
| todoId | String | (Obligatoriskt) ID för posten som ska uppdateras |  |
...
För att uppdatera värden för anpassade fält, använd mutation `setTodoCustomField` med fältspecifika parametrar:
...
### Noter
1. ID för anpassade fält kan hittas med hjälp av frågan [lista anpassade fält](/api/custom-fields/list-custom-fields)
2. Telefonnummer måste vara i E.164-format när API:et används direkt

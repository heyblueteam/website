...
- `false` - Operation failed (with error details)
...
**Note:** You cannot remove users with `OWNER` role from a project.
...
**Note:** Only company owners can remove users from the company.
...
- Sends email notification to the removed user
...
{
  "errors": [{
    "message": "User not found or not in project",
    "extensions": {
      "code": "USER_NOT_FOUND"
    }
  }]
...
- **Notification**: Only company-level removal sends an email notification to the removed user.

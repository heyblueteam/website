...
## Membaca Data
Anda dapat menyalin dan menempelkan perintah kode curl berikut ke terminal Anda untuk memulai: .
...
```
Atau coba contoh Python atau Node kami.
...
@@CB##fb6c40e3-49d5-40d8-8799-519e935508b7##CB@@json [Sample response for project list]
{
  "data": {
    "projectList": {
      "items": [
...
@@CB##6940e7bc-b39f-4324-a454-be47e3f49154##CB@@json [Sample response with more fields]
{
  "data": {
    "projectList": {
      "items": [
...
@@CB##146bfd30-ac76-461d-91f6-aabee5fd5a93##CB@@bash [curl]
curl -X POST https://api.blue.cc/graphql \
  -H "Content-Type: application/json" \
  -H "X-Bloo-Token-ID: YOUR_TOKEN_ID" \
  -H "X-Bloo-Token-Secret: YOUR_TOKEN_SECRET" \
...
@@CB##c25cf92a-00a7-46e3-a7cc-7ee94e6ff129##CB@@python [python]
import requests
...
print(response.json())  # To see the response
@@CB##c8a6fca5-2920-4f9c-84c8-218896e3a5da##CB@@javascript [node]
const axios = require('axios');
...
@@CB##eec7ed79-d58c-4204-b0f7-516130430dc1##CB@@

### Validation Error
When input parameters are invalid:

@@CB##7f8e460e-7bce-496f-888f-2f38fc285ffe##CB@@
...
Always check for the `errors` array in the response before processing `data`. If `errors` is present, the operation failed and `data` mungkin null.
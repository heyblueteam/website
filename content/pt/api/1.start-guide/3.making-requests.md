...
## Lendo Dados
Você pode copiar e colar o seguinte comando curl em seu terminal para começar: .
...
```
Ou experimente nossos exemplos em Python ou Node.
...
const data = {
  query: `query ProjectListQuery { projectList(filter: { companyIds: ["seu-id-de-empresa"] }) { items {name} } }`
};
...
print(response.json())  # To see the response
@@CB##e183c830-b5a4-47cf-9352-1261596cae56##CB@@javascript [node]
const axios = require('axios');
...
const data = {
  query: `mutation CreateRecord { createTodo(input: { todoListId: "TODOLISTID", title: "Teste", position: 65535 }) { id title position } }`
};
...
print(response.json())  # To see the response
@@CB##1792731c-9248-42ba-9154-7918c526cc83##CB@@javascript [node]
const axios = require('axios');
...
const data = {
  query: `mutation DeleteARecord { deleteTodo(input: { todoId: "DIGITE_O_ID_DO_REGISTRO" }) { success } }`
};
...
@@CB##904abab7-2f66-42d6-8dad-feb5ca696e53##CB@@
...
@@CB##79d5b1a2-ba91-4192-969d-0283490cd266##CB@@
...
@@CB##252c1b22-9c1f-4cfb-b43b-234763062018##CB@@

Always check for the `errors` array in the response before processing `data`. If `errors` is present, the operation failed and `data` pode ser nulo.
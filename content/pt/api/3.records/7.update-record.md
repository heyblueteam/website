...
description: Aprenda como atualizar detalhes de registro e campos personalizados usando a API do Blue.
---
...
| todoListId | String | Novo ID da lista se movendo o registro |  |
...
Para atualizar os valores dos campos personalizados, use a mutação `setTodoCustomField` com parâmetros específicos do campo:
...
Permissões adicionais para campos personalizados podem se aplicar para `setTodoCustomField` com base na configuração de funções.
...
### Notas
1. Os IDs de campos personalizados podem ser encontrados usando a consulta [listar campos personalizados](/api/custom-fields/list-custom-fields)
2. Os números de telefone devem estar no formato E.164 ao usar a API diretamente
3. Os campos de localização são melhor gerenciados através da interface do aplicativo Blue

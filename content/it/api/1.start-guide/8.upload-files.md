---
title: Caricamento di File
description: Guida completa per il caricamento di file su Blue utilizzando mutazioni GraphQL o REST API
---

## Panoramica

Questa guida dimostra come caricare file su Blue utilizzando due approcci diversi:

1. **Caricamento Diretto GraphQL** (Consigliato) - Caricamento semplice in un solo passaggio con un limite di dimensione del file di 256MB
2. **Caricamento REST API** - Processo in tre passaggi che supporta file di dimensioni maggiori fino a 4.8GB

Questa è una comparazione dei due metodi:

| Caratteristica | Caricamento GraphQL | Caricamento REST API |
|----------------|---------------------|-----------------------|
...
## Caricamento File GraphQL

Il metodo di caricamento GraphQL fornisce un modo semplice e diretto per caricare file con una singola richiesta.

### uploadFile

Carica un singolo file nel sistema di archiviazione dei file e crea un riferimento al file nel database.

**Input:**
- `file: Upload!` - Il file da caricare (utilizzando multipart/form-data)
- `projectId: String!` - ID del progetto o slug dove il file sarà archiviato
- `companyId: String!` - ID dell'azienda o slug dove il file sarà archiviato

**Restituisce:** `File!` - L'oggetto file creato
...
Carica più file nel sistema di archiviazione dei file e crea riferimenti ai file nel database.

**Input:**
- `files: [Upload!]!` - Array di file da caricare (max 10)
- `projectId: String!` - ID del progetto o slug dove i file saranno archiviati
- `companyId: String!` - ID dell'azienda o slug dove i file saranno archiviati

**Restituisce:** `[File!]!` - Array di oggetti file creati
...
@@CB##677a85eb-bf55-4091-b9e7-e9a556d2aeee##CB@@html
<!-- HTML -->
<input type="file" id="fileInput" />
<button onclick="uploadFile()">Upload File</button>
@@CB##1ce05f61-e886-4686-86a2-fb8a586e9e77##CB@@javascript

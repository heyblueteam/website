...
## Gegevens Lezen
Je kunt de volgende curl-opdracht kopiëren en plakken in je terminal om aan de slag te gaan: .
...
```
Of probeer onze Python of Node voorbeelden.
...
@@CB##b034d3bc-4392-4352-a9fd-55ad85bcb9f1##CB@@json [Sample response for project list]
{
  "data": {
    "projectList": {
      "items": [
...
@@CB##5c174b2d-1334-4bc7-91cd-a77e8336fe95##CB@@bash [curl]
curl -X POST https://api.blue.cc/graphql \
  -H "Content-Type: application/json" \
...
@@CB##86908b95-9ab3-4c1d-91a0-c47371a11c42##CB@@python [python]
import requests
...
print(response.json())  # To see the response
@@CB##f061ad2c-8b17-4231-8e26-9b542537a178##CB@@javascript [node]
const axios = require('axios');
...
@@CB##46f83830-9912-44e5-821d-74ed3395f1e6##CB@@bash [curl]
curl -X POST https://api.blue.cc/graphql \
  -H "Content-Type: application/json" \
  -H "X-Bloo-Token-ID: YOUR_TOKEN_ID" \
  -H "X-Bloo-Token-Secret: YOUR_TOKEN_SECRET" \
...
@@CB##36706cd1-f9d7-4a27-86d3-be352b02e30d##CB@@python [python]
import requests
...
print(response.json())  # To see the response
@@CB##e41b2b6f-df6b-4066-ad1f-404eacbe3f7d##CB@@javascript [node]
const axios = require('axios');
...
@@CB##f060e5b9-6e9d-46a9-870e-d26894ff9196##CB@@

### Permission Error
When you don't have access to perform an operation:

@@CB##09ba3ea2-b37e-4a8e-b493-03f27af0ff14##CB@@

### Rate Limit Error
When you exceed the API rate limits:

@@CB##33cfc277-13b7-45c3-aa16-80b9d1431dfd##CB@@

Always check for the `errors` array in the response before processing `data`. If `errors` is present, the operation failed and `data` kan null zijn.
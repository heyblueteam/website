...
| startedAt/duedAt | DateTime | ISO 8601形式の更新された開始日/終了日 |  |
...
追加のカスタムフィールド権限は、役割構成に基づいて`setTodoCustomField`に適用される場合があります。

## 戻り値

- `editTodo`は、完全に更新された`Todo`オブジェクトを返します
- `setTodoCustomField`は、成功を示す`Boolean!`を返します

### 注意事項
1. カスタムフィールドIDは、[カスタムフィールドのリスト](/api/custom-fields/list-custom-fields)クエリを使用して見つけることができます
2. 電話番号は、APIを直接使用する場合、E.164形式である必要があります
3. 位置フィールドは、Blueアプリインターフェースを通じて管理するのが最適です
4. `html`および`text`フィールドは、`html`が提供されると自動的に同期されます
5. すべての更新操作は、アクティビティログとWebhook通知をトリガーします